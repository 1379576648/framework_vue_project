  ...
  <div class="body_1">
    <el-tabs type="border-card">
      ...
      <el-tab-pane label="待办申请">
        <el-button @click="selectWorkerlAll2()">重置</el-button>
        &nbsp;
        <el-input
            v-model="staffName"
            placeholder="根据申请人搜索"
        ...
        &nbsp;
        <el-button type="success" plain @click="selectWorkerlAll()">搜索</el-button>
        <!--  表格 -->
        <el-table
            ref="filterTable1"
            row-key="date1"
      ...
      <el-drawer v-model="drawer" title="I am the title" :with-header="false">
        <el-form ref="form" :model="details">
          ...
          <el-form-item label="转正类型：">
            <el-input v-model="details[0].workertype" disabled></el-input>
          </el-form-item>
          <el-form-item label="备注：">
            <el-input v-model="details[0].workerremarks" disabled></el-input>
          </el-form-item>
          <el-form-item label="转正日期：">
            <el-input v-model="details[0].workerdate" disabled></el-input>
          </el-form-item>
      ...
      <el-tab-pane label="已办申请">
        <el-button @click="selectEndWorkerlAll2()">重置</el-button>
        &nbsp;
        <el-input
            v-model="staffName1"
            placeholder="根据申请人搜索"
        ...
        &nbsp;
        <el-button type="success" plain @click="selectEndWorkerlAll()">搜索</el-button>
        <el-table
            ref="filterTable"
            row-key="date"
            :data="tableData1"
        ...
        <el-drawer v-model="drawer2" title="I am the title" :with-header="false">
          <el-form ref="form" :model="details2">
            ...
            <el-form-item label="转正类型：">
              <el-input v-model="details2[0].workertype" disabled></el-input>
            </el-form-item>
            <el-form-item label="备注：">
              <el-input v-model="details2[0].workerremarks" disabled></el-input>
            </el-form-item>
            <el-form-item label="转正日期：">
              <el-input v-model="details2[0].workerdate" disabled></el-input>
            </el-form-item>
            <el-form-item label="当前审核状态：">
        ...
        <div class="demo-pagination-block">
          <el-pagination v-model:current-page="pageInfo1.currentPage"
                         ...
                         next-text="下一页"
                         prev-text="上一页"
                         @size-change="selectEndWorkerlAll()"
                         @current-change="selectEndWorkerlAll()"
                         @prev-click="selectEndWorkerlAll()"
                         @next-click="selectEndWorkerlAll()">
          </el-pagination>
        </div>
      </el-tab-pane>
    </el-tabs>
...
export default {
  ...
  methods: {
    ...
    selectEndWorkerlAll() {
      ...
      this.axios({
        method: 'post',
        url: this.url + 'selectEndWorkerlAll',
        data: {
          //当前页
    ...
    // 备注弹出框点击确定 通过当前审批 (一个审批人) 最后一个审批人点击通过，需完成对应的审批操作
    pass_overtime2() {
      var _this = this
      this.axios({
        method: 'post',
